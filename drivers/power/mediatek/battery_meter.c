...
kal_int32 g_battery_id_voltage[] = {100000,-1};
int TOTAL_BATTERY_NUMBER = 2;
int BATTERY_ID_CHANNEL_NUM = 12;
...
//liqiang 20150509 begin
#ifdef CONFIG_WIND_DEVICE_INFO
extern int battery_boot_data[10];
#endif
//liqiang 20150509 end
...
//liqiang 20150509 begin
#ifdef CONFIG_WIND_DEVICE_INFO
	battery_boot_data[0] = gFG_voltage;
	battery_boot_data[1] = gFG_capacity_by_v;
	battery_boot_data[2] = gFG_capacity_by_v_init;
	battery_boot_data[3] = g_rtc_fg_soc;
#endif	
//liqiang 20150509 end
...
//liqiang 20150509 begin
 #ifdef CONFIG_WIND_DEVICE_INFO
	battery_boot_data[4] = gFG_capacity_by_c;
	battery_boot_data[5] = gFG_DOD0;
	battery_boot_data[6] = gFG_15_vlot;
 #endif
//liqiang 20150509 end
...
//liqiang 20150509 begin
    ...
    if(count < 10)
    {
	    bm_print(BM_LOG_CRTI, "[battery_boot]print battery boot info count = %d times\n", count);
		bm_print(BM_LOG_CRTI, "[battery_boot]get_hw_ocv = %d\n", battery_boot_data[0]);
		bm_print(BM_LOG_CRTI, "[battery_boot]HW_SOC = %d\n", battery_boot_data[1]);
		bm_print(BM_LOG_CRTI, "[battery_boot]SW_SOC = %d\n", battery_boot_data[2]);
		bm_print(BM_LOG_CRTI, "[battery_boot]rtc_fg_soc = %d\n", battery_boot_data[3]);
		bm_print(BM_LOG_CRTI, "[battery_boot]gFG_capacity_by_c = %d\n", battery_boot_data[4]);
		bm_print(BM_LOG_CRTI, "[battery_boot]gFG_DOD0 = %d\n", battery_boot_data[5]);
		bm_print(BM_LOG_CRTI, "[battery_boot]gFG_15_vlot = %d\n", battery_boot_data[6]);	
		bm_print(BM_LOG_CRTI, "[battery_boot]is_battery_remove = %d\n", is_battery_remove_pmic());	
		count++;
    }
 #endif
//liqiang 20150509 end
