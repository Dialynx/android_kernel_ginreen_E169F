...
//liuying 20150808 add begin
#define S5K3l2_USE_AWB_OTP
#ifdef S5K3l2_USE_AWB_OTP

kal_uint16 read_3l2_eeprom_reg(kal_uint32 addr)
{
    kdSetI2CSpeed(imgsensor_info.i2c_speed);
     kal_uint16 get_byte=0;
    char pusendcmd[2] = {(char)(addr >> 8) , (char)(addr & 0xFF) };
...
//#define S5K3l2OTP_WRITE_ID         0x20
...
//	
//	char puSendCmd[2] = {(char)(addr >> 8) , (char)(addr & 0xFF) };
//	iReadRegI2C(puSendCmd , 2, (u8*)&get_byte, 1, S5K3l2OTP_WRITE_ID);
//	return get_byte&0x00ff;
//}
...
extern OTP_DATA s5k3l2_eeprom_data;

void S5K3l2_MIPI_read_otp_wb(struct S5K3l2_MIPI_otp_struct *otp)
{
  	...
  	//if(FLG==0)
		//S5K3l2DB("No OTP Data or OTP data is invalid");
   	//else
   	{     
		/*
		golden_R = read_3l2_eeprom_reg(0x0010);           	 // read Golden value
		golden_Gr= read_3l2_eeprom_reg(0x0011);              // read Golden value
 		golden_Gb= read_3l2_eeprom_reg(0x0012);              // read Golden value
		golden_B = read_3l2_eeprom_reg(0x0013);              // read Golden value
  		LOG_INF("[S5K3l2] [S5K3l2_MIPI_read_otp_wb] golden_R=0x%x, golden_Gr=0x%x, golden_Gb=0x%x, golden_B=0x%x\n", golden_R, golden_Gr, golden_Gb, golden_B);
  		//write_cmos_sensor(0x3a02, 0x00); //page set  
  		//write_cmos_sensor(0x3a00, 0x01); //otp enable read
  		current_R =  read_3l2_eeprom_reg(0x000c);
  		current_Gr = read_3l2_eeprom_reg(0x000d);
  		current_Gb = read_3l2_eeprom_reg(0x000e);
  		current_B =  read_3l2_eeprom_reg(0x000f); 
  		LOG_INF("[S5K3l2] [S5K3l2_MIPI_read_otp_wb] current_R=0x%x, current_Gr=0x%x, current_Gb=0x%x, current_B=0x%x\n", current_R, current_Gr, current_Gb, current_B);
   		...
   		otp->b_ratio = b_ratio;
   		*/
 kal_uint16  current_rg_ratio, current_bg_ratio, golden_rg_ratio, golden_bg_ratio;


 //current_rg_ratio = (read_3l2_eeprom_reg(0x0023)<<8) + read_3l2_eeprom_reg(0x0024);
 //current_bg_ratio = (read_3l2_eeprom_reg(0x0025)<<8) + read_3l2_eeprom_reg(0x0026);
 //golden_rg_ratio = (read_3l2_eeprom_reg(0x0029)<<8) + read_3l2_eeprom_reg(0x0030);
 //golden_bg_ratio = (read_3l2_eeprom_reg(0x0031)<<8) + read_3l2_eeprom_reg(0x0032);
 current_rg_ratio =  ((s5k3l2_eeprom_data.Data[23])<<8)+(s5k3l2_eeprom_data.Data[24]);
 current_bg_ratio =  ((s5k3l2_eeprom_data.Data[25])<<8)+(s5k3l2_eeprom_data.Data[26]);
 ...
 otp->r_ratio = 512*golden_rg_ratio/current_rg_ratio;
 otp->b_ratio = 512*golden_bg_ratio/current_bg_ratio;
 
 
 LOG_INF("liuying OTP current_rg_ratio =%x,current_bg_ratio=%x,golden_rg_ratio=%x golden_bg_ratio=%x\n",current_rg_ratio,current_bg_ratio,golden_rg_ratio,golden_bg_ratio);

	
 LOG_INF("[S5K3l2] [S5K3l2_MIPI_read_otp_wb] r_ratio=0x%x, b_ratio=0x%x\n", otp->r_ratio, otp->b_ratio);      
   	}
}



void S5K3l2_MIPI_algorithm_otp_wb1(struct S5K3l2_MIPI_otp_struct *otp)
{
   ...
   LOG_INF("[S5K3l2] [S5K3l2_MIPI_algorithm_otp_wb1] R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", R_GAIN, B_GAIN,G_GAIN);    
}



void S5K3l2_MIPI_write_otp_wb(struct S5K3l2_MIPI_otp_struct *otp)
{
   ...
   write_cmos_sensor(0x020f, (G_GAIN&0xff)); // g_gain
   write_cmos_sensor(0x0210, (R_GAIN>>8)); // r_gain
   write_cmos_sensor(0x0211, (R_GAIN&0xff)); // r_gain
   write_cmos_sensor(0x0212, (B_GAIN>>8)); // b_gain
   write_cmos_sensor(0x0213, (B_GAIN&0xff)); // b_gain 
   write_cmos_sensor(0x0214, (G_GAIN>>8)); // g_gain
   write_cmos_sensor(0x0215, (G_GAIN&0xff)); // g_gain 
}

void S5K3l2_MIPI_update_wb_register_from_otp(void)
{
   struct S5K3l2_MIPI_otp_struct current_otp;
   
   S5K3l2_MIPI_read_otp_wb(&current_otp);
   S5K3l2_MIPI_algorithm_otp_wb1(&current_otp);
   S5K3l2_MIPI_write_otp_wb(&current_otp);
}

#endif
//liuying 20150808 add end
...
{
	LOG_INF("s5k3l2 sensor_init Enter\n");
  ...
  write_cmos_sensor1(0x6028, 0xD000);
	...
	write_cmos_sensor1(0x6F12, 0x0001);
	LOG_INF("s5k3l2 sensor_init Exit\n");
}    /*    sensor_init  */
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
		  ...
		  LOG_INF("liuying read 3l2 eeprom data done\n");
                #endif
                return ERROR_NONE;
            }
...
static kal_uint32 open(void)
{
    ...
    write_cmos_sensor1(0x602A, 0x0100);
    write_cmos_sensor(0x6F12, 0x01);
    S5K3l2_MIPI_update_wb_register_from_otp();
    //liuying 20150808 add end
