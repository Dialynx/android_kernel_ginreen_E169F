...
static imgsensor_info_struct imgsensor_info = { 
	...
	.cap_delay_frame = 1, //liuying 20150825
	.pre_delay_frame = 1, //liuying 20150825
	.video_delay_frame = 1,//liuying 20150825
	.hs_video_delay_frame = 1,//liuying 20150825
	.slim_video_delay_frame = 1,//liuying 20150825
...
{{ 3280, 2464,  0,	  0,   3280, 2464, 1640, 1232, 0000, 0000, 1640, 1232,	0,	0, 1640, 1232}, // Preview 
 { 3280, 2464,	  0,	  0,   3280, 2464, 3264, 2448, 0000, 0000, 3264, 2448,	0,	0, 3264, 2448}, // capture 
 { 3280, 2464,	  0,	  0,   3280, 2464, 1640, 1232, 0000, 0000, 1640, 1232,	0,	0, 1640, 1232}, // video 
...
{ 
		...
		write_cmos_sensor(0x0101,0x03);//liuying 20150724
...
{              LOG_INF("E//VGA 120fps");
			...
			write_cmos_sensor(0x0101,0x03);       //Image  orientation	image_orientation ([0] mirror en, [1] flip en) //liuying 20150724
...
static void slim_video_setting()
	{
		...
		//Image  Orientation -------------------------------
		write_cmos_sensor(0x0101,0x03); 	//	[1:0]	image_orientation ([0] mirror en, [1] flip en) //liuying 20150724
...
//liuying 20150716 modify end
//liuying 20150727 add begin
#define S5K4h5yc_USE_AWB_OTP

#if defined(S5K4h5yc_USE_AWB_OTP)

#define GAIN_DEFAULT       	     0x0100
#define S5K4h5ycOTP_WRITE_ID         0x20
...
//	iReadRegI2C(puSendCmd , 2, (u8*)&get_byte, 1, S5K4h5ycOTP_WRITE_ID);
...
void S5K4h5yc_MIPI_read_otp_wb(struct S5K4h5yc_MIPI_otp_struct *otp)
{
   	...
   	//liuying  20150728 begin 
   	kal_uint16 Flag = 0x00;
   
   	
   
  	//if(FLG==0)
		//S5K4h5ycDB("No OTP Data or OTP data is invalid");
   	//else
   	{
		write_cmos_sensor(0x3a02, 0x00); //page set  
  		write_cmos_sensor(0x3a00, 0x01); //otp enable read

		Flag = read_cmos_sensor(0x3a0d);

		if(Flag == 0x40)//group 1
		{
			golden_R = read_cmos_sensor(0x3a12);           	  // wait for update
			golden_Gr= read_cmos_sensor(0x3a13);              // wait for update
 			golden_Gb= read_cmos_sensor(0x3a14);              // wait for update
			golden_B = read_cmos_sensor(0x3a15);              // wait for update
			LOG_INF("[S5K4h5yc] [S5K4h5yc_MIPI_read_otp_wb] golden_R=0x%x, golden_Gr=0x%x, golden_Gb=0x%x, golden_B=0x%x\n", golden_R, golden_Gr, golden_Gb, golden_B);
			...
			current_R =  read_cmos_sensor(0x3a0e);
  			...
  			LOG_INF("[S5K4h5yc] [S5K4h5yc_MIPI_read_otp_wb] current_R=0x%x, current_Gr=0x%x, current_Gb=0x%x, current_B=0x%x\n", current_R, current_Gr, current_Gb, current_B);
		}
		else if(Flag == 0xd0)//group 2
		{
			golden_R = read_cmos_sensor(0x3a1a);           	  // wait for update
			golden_Gr= read_cmos_sensor(0x3a1b);              // wait for update
 			golden_Gb= read_cmos_sensor(0x3a1c);              // wait for update
			golden_B = read_cmos_sensor(0x3a1d);              // wait for update
			LOG_INF("[S5K4h5yc] [S5K4h5yc_MIPI_read_otp_wb] golden_R=0x%x, golden_Gr=0x%x, golden_Gb=0x%x, golden_B=0x%x\n", golden_R, golden_Gr, golden_Gb, golden_B);
		...
		//LOG_INF("liuying test 4h5 otp 3a04=0x%x 3a05=0x%x 3a06=0x%x 3a07=0x%x 3a08=0x%x 3a09=0x%x\n",
		//read_cmos_sensor(0x3a04),read_cmos_sensor(0x3a05),read_cmos_sensor(0x3a06),read_cmos_sensor(0x3a07),read_cmos_sensor(0x3a08),
		//read_cmos_sensor(0x3a09));
		//LOG_INF("liuying test 4h5 otp 3a0a=0x%x 3a0b=0x%x 3a0c=0x%x 3a0d=0x%x \n",
		//read_cmos_sensor(0x3a0a),read_cmos_sensor(0x3a0b),read_cmos_sensor(0x3a0c),read_cmos_sensor(0x3a0d));
		//liuying  20150728 end 
   		...
   		LOG_INF("[S5K4h5yc] [S5K4h5yc_MIPI_read_otp_wb] r_ratio=0x%x, b_ratio=0x%x\n", otp->r_ratio, otp->b_ratio);      
   	}
}



void S5K4h5yc_MIPI_algorithm_otp_wb1(struct S5K4h5yc_MIPI_otp_struct *otp)
{
...
void S5K4h5yc_MIPI_write_otp_wb(struct S5K4h5yc_MIPI_otp_struct *otp)
{
   ...
   write_cmos_sensor_8(0x020f, G_GAIN & 0xFFFF); // g_gain
   write_cmos_sensor_8(0x0210, R_GAIN >> 8); // r_gain
   write_cmos_sensor_8(0x0211, R_GAIN & 0xFFFF); // r_gain
   write_cmos_sensor_8(0x0212, B_GAIN >> 8); // b_gain
   write_cmos_sensor_8(0x0213, B_GAIN & 0xFFFF); // b_gain 
   write_cmos_sensor_8(0x0214, G_GAIN >> 8); // g_gain
   write_cmos_sensor_8(0x0215, G_GAIN & 0xFFFF); // g_gain 
}



void S5K4h5yc_MIPI_update_wb_register_from_otp(void)
{
   struct S5K4h5yc_MIPI_otp_struct current_otp;
   
   S5K4h5yc_MIPI_read_otp_wb(&current_otp);
   S5K4h5yc_MIPI_algorithm_otp_wb1(&current_otp);
   S5K4h5yc_MIPI_write_otp_wb(&current_otp);
}
#endif
//liuying 20150727 add end
...
static kal_uint32 open(void)
{
	...
	printk("liuying test 4h5 awb otp\n");
	#endif
	//liuying 20150727 end
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
	     //liuying 20150826 add begin
            get_default_framerate_by_scenario((MSDK_SCENARIO_ID_ENUM)*(feature_data), (MUINT32 *)(uintptr_t)(*(feature_data+1)));           
	     //liuying 20150826 add end
			break;
