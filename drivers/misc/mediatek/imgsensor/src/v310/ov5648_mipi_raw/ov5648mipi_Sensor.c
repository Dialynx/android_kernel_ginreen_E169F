...
static imgsensor_info_struct imgsensor_info = {
    ...
    .ae_ispGain_delay_frame = 2,//isp gain delay frame for AE cycle //zhenglihong 20150927
    .ihdr_support = 0,      //1, support; 0,not support
    .ihdr_le_firstline = 0,  //1,le first ; 0, se first
    .sensor_mode_num = 5,      //support sensor mode num

    .cap_delay_frame = 1,//liuying 20150523
    .pre_delay_frame = 1,//liuying 20150523
    .video_delay_frame = 1,//liuying 20150523
...
//liuying 20150803 add begin
#if defined OV5648OTP
...
static int check_otp(int index)
{
	...
	LOG_INF("check_otp() = 0x%x, 0x%x, 0x%x\r\n", flag, rg, bg);

	if (flag==0x09) {  //otp in this index
		return 1;
	}
	else
	{
...
static int read_otp(int index, struct otp_struct *otp_ptr)
{
	...
	if (index == 1)
	{
		...
		temp = read_cmos_sensor(0x3d0c);
		(*otp_ptr).user_data[0] = (read_cmos_sensor(0x3d09)<<2) + ((temp>>6) & 0x03);
		(*otp_ptr).user_data[1] = (read_cmos_sensor(0x3d0a)<<2) + ((temp>>6) & 0x03);
	}
	else if (index == 2)
	{
		// read otp --Bank 0
		...
		temp = read_cmos_sensor(0x3d05);
		(*otp_ptr).user_data[0] = (read_cmos_sensor(0x3d02)<<2) + ((temp>>6) & 0x03);
		(*otp_ptr).user_data[1] = (read_cmos_sensor(0x3d03)<<2) + ((temp>>6) & 0x03);
	}
	else if (index == 3)
	{
		// read otp --Bank 1
		...
		(*otp_ptr).rg_ratio = (read_cmos_sensor(0x3d09)<<2) + ((temp>>6) & 0x03);
		(*otp_ptr).bg_ratio = (read_cmos_sensor(0x3d0a)<<2) + ((temp>>4) & 0x03);
		(*otp_ptr).light_rg = 0;//(read_cmos_sensor(0x3d0e)<<2) + ((temp>>2) & 0x03);
		(*otp_ptr).light_bg = 0;//(read_cmos_sensor(0x3d0f)<<2) + (temp & 0x03);
		temp = read_cmos_sensor(0x3d0e);
		(*otp_ptr).user_data[0] = (read_cmos_sensor(0x3d0b)<<2) + ((temp>>6) & 0x03);
		(*otp_ptr).user_data[1] = (read_cmos_sensor(0x3d0c)<<2) + ((temp>>6) & 0x03);
	}

	LOG_INF("integrator_id = %x\r\n", (*otp_ptr).module_integrator_id );
	LOG_INF("lens_id = 0x%x rg_ratio=0x%x bg_ratio=0x%x\n", (*otp_ptr).lens_id , (*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio);
...
static int RG_Ratio_Typical = 0x12e;//0x19f;
static int BG_Ratio_Typical = 0x128;//0x163;
...
static int update_otp(void)
{
	...
	//liuying 20150717 begin
	RG_Ratio_Typical=current_otp.user_data[0];
	BG_Ratio_Typical=current_otp.user_data[1];
	//liuying 20150717 end
	...
	//before OTP update
	LOG_INF("reg[0x5186] = %x\r\n", read_cmos_sensor(0x5186));
	LOG_INF("reg[0x5187] = %x\r\n", read_cmos_sensor(0x5187));
	LOG_INF("reg[0x5188] = %x\r\n", read_cmos_sensor(0x5188));
	LOG_INF("reg[0x5189] = %x\r\n", read_cmos_sensor(0x5189));
	LOG_INF("reg[0x518a] = %x\r\n", read_cmos_sensor(0x518a));
	LOG_INF("reg[0x518b] = %x\r\n", read_cmos_sensor(0x518b));
...
{
    LOG_INF("liuying sensor_init Enter\n");
    /*****************************************************************************
    ...
    */
    ...
    // Horizontal binning
    //liuying 20150717 add begin
    write_cmos_sensor(0x3820, 0x08); // flip off, v bin off      //0x08
    write_cmos_sensor(0x3821, 0x07); // mirror on, h bin on   //0x07
    //liuying 20150717 add end
    ...
    LOG_INF("liuying sensor_init exit\n");
}    /*    MIPI_sensor_Init  */
...
static void capture_setting(kal_uint16 currefps)
{
    ...
    /********************************************************
       ...
       ********************************************************/
//liuying  20150717 begin
    write_cmos_sensor(0x3820, 0x40); // flip off, v bin off        //0x40
    write_cmos_sensor(0x3821, 0x06); // mirror on, v bin off    //0x06
//liuying  20150717 end
...
static kal_uint32 open(void)
{
    ...
    LOG_1;
    LOG_2;
     LOG_INF("liuying open sensor enter\n");
    //sensor have two i2c address 0x6c 0x6d & 0x21 0x20, we should detect the module used i2c address
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        spin_lock(&imgsensor_drv_lock);
        imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
...
//liuying 20150303 add begin
LOG_INF("liuying otp begin\n");
#if defined OV5648OTP
     update_otp();
#endif
LOG_INF("liuying otp end\n");
//liuying 20150303 add end
   ...
   LOG_INF("liuying open sensor exit\n");
