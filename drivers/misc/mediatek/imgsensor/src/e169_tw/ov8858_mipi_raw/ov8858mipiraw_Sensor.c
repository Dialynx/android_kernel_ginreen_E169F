...
#include <linux/slab.h>   //wangkangmin add 0901
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.i2c_addr_table = {0x42,0xff},//record sensor support all write id addr, only supprt 4must end with 0xff   //liuying 20150729
};
...
//  ;;
	...
	write_cmos_sensor(0x3820, 0x06);   //liuying 20150731 0x00
	write_cmos_sensor(0x3821, 0x61);   //liuying 20150731 0x67
...
//   100 99 1632 1224
	...
	write_cmos_sensor(0x3814, 0x03);     //
	//write_cmos_sensor(0x3821, 0x67);     //  //liuying 20150731
	write_cmos_sensor(0x382a, 0x03);     //
	write_cmos_sensor(0x382b, 0x01);     //
	write_cmos_sensor(0x3830, 0x08);     //
	write_cmos_sensor(0x3836, 0x02);     //
...
{
	LOG_INF("capture_setting Enter! currefps:%d\n",currefps);
	if (currefps == 150) {
...
//  100 99 3264 2448);    //
	...
	write_cmos_sensor(0x3821, 0x40);  //  //liuying 20150731 0x46  
...
//  100 99 640 480
	...
	write_cmos_sensor(0x3814, 0x03);  //
	write_cmos_sensor(0x3821, 0x69);  //  liuying 20150731 0x6f
...
int read_otp(struct otp_struct *otp_ptr)
{
	...
	if(addr != 0) {
		...
		LOG_INF("wangkangmin ov8858 module_integrator_id=%x\n",(*otp_ptr).module_integrator_id);
		LOG_INF("wangkangmin ov8858 temp=%x\n",temp);
		LOG_INF("wangkangmin ov8858 rg_ratio=%x\n",(*otp_ptr).rg_ratio);
		LOG_INF("wangkangmin ov8858 bg_ratio=%x\n",(*otp_ptr).bg_ratio);
	}
	else {
...
{
	int RG_Ratio_Typical = 0x15d, BG_Ratio_Typical = 0x142;
...
static kal_uint32 open(void)
{
	...
	LOG_INF("open enter PLATFORM:MT6735,MIPI 4LANE\n");
