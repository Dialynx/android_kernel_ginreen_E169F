...
int read_otp(struct otp_struct *otp_ptr)
{
	...
	if(addr != 0) {
		...
		LOG_INF("liukun ov8858 module_integrator_id=%x\n",(*otp_ptr).module_integrator_id);
		LOG_INF("liukun ov8858 temp=%x\n",temp);
		LOG_INF("liukun ov8858 rg_ratio=%x\n",(*otp_ptr).rg_ratio);
		LOG_INF("liukun ov8858 bg_ratio=%x\n",(*otp_ptr).bg_ratio);
	}
	else {
...
{
	int RG_Ratio_Typical = 0x123, BG_Ratio_Typical = 0x100;
