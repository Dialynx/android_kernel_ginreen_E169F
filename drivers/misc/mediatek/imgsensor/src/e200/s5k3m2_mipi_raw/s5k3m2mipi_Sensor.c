//zhenglihong 20150923 add begin
/*****************************************************************************
 *
 * Filename:
 * ---------
...
static imgsensor_info_struct imgsensor_info = { 
	...
	//liuying 20150409 begin 20150409 for mms cannot video
	.normal_video = { 
		.pclk = 440000000,				
		.linelength = 4592,				
		.framelength =3188, 
	...
	//liuying 20150409 end 20150409 for mms cannot video
	.hs_video = {
		.pclk = 440000000,
		.linelength = 4592,
		.framelength = 798,
	...
	.cap_delay_frame = 3,//modify for long explorse 2, 
	.pre_delay_frame = 2,  
	.video_delay_frame = 2,
    ...
    .i2c_speed = 400,//300, // i2c read/write speed vivo zcw++ optimize open camera Speed
};
...
//liuying 20150425 begin
//zhenglihong 20150825 changed for pd info begin
#if 1
...
#endif
//zhenglihong 20150825 changed for pd info end

//liuying 20150425 end
...
//zhenglihong 20150825 changed for awb calibration begin
void S5K3m2_MIPI_read_otp_wb(struct S5K3m2_MIPI_otp_struct *otp)
{
  	...
  	//if(FLG==0)
   	...
   	//else
   	{
		golden_R = read_3m2_eeprom_reg(0x0023);           	 // read Golden value
		golden_Gr= read_3m2_eeprom_reg(0x0024);              // read Golden value
 		golden_Gb= read_3m2_eeprom_reg(0x0025);              // read Golden value
		golden_B = read_3m2_eeprom_reg(0x0026);              // read Golden value
  		...
  		current_R =  read_3m2_eeprom_reg(0x001F);
  		current_Gr = read_3m2_eeprom_reg(0x0020);
  		current_Gb = read_3m2_eeprom_reg(0x0021);
  		current_B =  read_3m2_eeprom_reg(0x0022); 
...
//zhenglihong 20150825 changed for awb calibration end
...
static void write_shutter(unsigned long long shutter)
{
		...
		if(shutter >= 0xFFF0 )   // need to modify line_length & PCLK
	{
		...
		shutter = 0xFFF0;       
		LOG_INF("pclk = %d, shutter = %d, imgsensor.line_length = %d, exposure_time = %d\n", imgsensor.pclk, shutter, imgsensor.line_length,exposure_time);
		...
		if(exposure_time <= MAX_TIME1)  // only need to modify line_length
		{
			...
			LOG_INF("pclk = %d, shutter = %d, line_length = %d, exposure_time = %d\n", imgsensor.pclk, shutter, line_length1,exposure_time);
		}
...
//	otp_lsc_update();
  ...
  write_cmos_sensor(0x3AFA, 0xFBF8);  //LP mode in V-blank 0310
...
write_cmos_sensor(0x6F12,0x0040);
write_cmos_sensor(0x6F12,0x0040);
write_cmos_sensor(0x602A,0x192C);//+///Close BPC
write_cmos_sensor(0x6F12,0x0001);//+///Close BPC
write_cmos_sensor(0x6028,0x4000);
write_cmos_sensor(0x0344,0x0010);//M///Close BPC 0x000C
write_cmos_sensor(0x0346,0x0010);//M///Close BPC 0x000C
write_cmos_sensor(0x0348,0x106F);//M///Close BPC 0x1073
write_cmos_sensor(0x034A,0x0C2F);//M///Close BPC 0x0C33
write_cmos_sensor(0x034C,0x1060);//
write_cmos_sensor(0x034E,0x0C20);
write_cmos_sensor_8(0x0901,0x11);
write_cmos_sensor(0x0380,0x0001);
...
write_cmos_sensor(0x0202,0x0200);
write_cmos_sensor(0x0200,0x0400);
write_cmos_sensor_8(0x0B05,0x00);//M///Close BPC 0x01
write_cmos_sensor_8(0x0B08,0x00);
write_cmos_sensor_8(0x3B3C,0x01);
write_cmos_sensor(0x3B34,0x3030);
write_cmos_sensor(0x3B36,0x3030);
...
write_cmos_sensor(0x0348,0x1073);
write_cmos_sensor(0x034A,0x0C33);
write_cmos_sensor(0x034C,0x1060);//zcw 0113 0160
write_cmos_sensor(0x034E,0x0C20);
write_cmos_sensor_8(0x0901,0x11);
write_cmos_sensor(0x0380,0x0001);
write_cmos_sensor(0x0382,0x0001);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	switch (scenario_id) {
		...
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			...
			preview(image_window, sensor_config_data);//liuying 20150409 for mms cannot video
			break;	  
		case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
