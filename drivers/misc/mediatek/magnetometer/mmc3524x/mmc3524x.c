...
static int sensor_data[16];//CALIBRATION_DATA_SIZE];
...
static int ECS_SaveData(int buf[16])
{
#if DEBUG	
	struct i2c_client *client = this_client;  
	struct mmc3524x_i2c_data *data = i2c_get_clientdata(client);
	...
	printk("Get daemon data: %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d,%d, %d, %d, %d!\n",
...
int mmc3524x_gyro_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout)
{
	//printk("Kaka mmc3524x_gyro_operate\n");
	int err = 0;
	int value;
...
/*----------------------------------------------------------------------------*/
static int mmc3524x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{       printk( "rooney222222222222welcometomsensor\n");
...
printk("[znw]%d\n",__LINE__);	
	if((err = misc_register(&mmc3524x_device)))
	{
		...
		printk( "rooney 1111 mmc3524x_device register ok\n");
...
/*----------------------------------------------------------------------------*/
static int mmc_probe(struct platform_device *pdev) 
{        printk( "rooney1111111111welcometomsensor\n");
	struct mag_hw *hw = mmc3524_get_cust_mag_hw();
