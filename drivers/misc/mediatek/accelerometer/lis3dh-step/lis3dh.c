...
static int lis3dsh_init_flag =-1; // 0<==>OK -1 <==> fail

static DEFINE_MUTEX(lis3dsh_init_mutex);
static unsigned long lis3dsh_init_flag_test = 0; //initial state
...
static int LIS3DSH_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if(enable == TRUE)
	{
		GSE_LOG("Sensor power status enable!\n");
	// 	acc_en = true;
		GSE_LOG("pedometer has not enable!\n");
...
0x00,
0x00,
0x02,                      //TH1 FOR WALKING PARAMETER
0x00,
0x00,
0x03,
0x21,
...
/*  Following Define for Running mode */
...
/*  FUNCTION FOR config RUNNING MODE  ********/
...
/*  FUNCTION FOR config WALKING MODE  ********/
...
static ssize_t show_pedometer_value(struct device_driver *ddri, char *buf)
{
	...
	lis3dsh_acc_get_pedometer_step(priv, &value);
	return snprintf(buf, PAGE_SIZE, "%x\n", value);        
}

static ssize_t store_pedometer_value(struct device_driver *ddri, const char *buf, size_t count)
{
	struct lis3dsh_i2c_data *obj = obj_i2c_data;
	struct lis3dsh_i2c_data *priv = obj_i2c_data;
	uint value;
	if (obj == NULL)
	{
	  ...
	  GSE_ERR(" input value \n");
	   lis3dsh_start_pedometer(priv,0x5);
...
static DRIVER_ATTR(pedo_value,       S_IWUSR | S_IRUGO, show_pedometer_value,      store_pedometer_value);
...
static int lis3dsh_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if((err = LIS3DSH_Init(new_client, 1)))
	{
...
static int  lis3dsh_remove(void)
{
    ...
    GSE_FUN();    
	clear_bit(LIS3DSH_ACC, &lis3dsh_init_flag_test);
    LIS3DSH_power(hw, 0);    
...
static int  lis3dsh_local_init(void)
{
   ...
   struct acc_hw *hw = lis3dsh_get_cust_acc_hw();
	...
	set_bit(LIS3DSH_ACC, &lis3dsh_init_flag_test);

	if(0==test_bit(LIS3DSH_STEP_C, &lis3dsh_init_flag_test))
	{
	    LIS3DSH_power(hw, 1);
...
static int lis3dsh_step_c_local_init(void)
{
    printk("Kaka lis3dsh_step_c_local_init \n ");
	int res = 0;
	...
	set_bit(LIS3DSH_STEP_C, &lis3dsh_init_flag_test);
	
	if(0==test_bit(LIS3DSH_ACC, &lis3dsh_init_flag_test)) 
	{
	   // LIS3DSH_power(hw, 1);
		if(i2c_add_driver(&lis3dsh_i2c_driver))
		{
...
static int lis3dsh_step_c_local_uninit(void)
{
	clear_bit(LIS3DSH_STEP_C, &lis3dsh_init_flag_test);
    return 0;
}
#endif
