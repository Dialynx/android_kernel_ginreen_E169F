...
static int LIS3DH_ReadSensorData_mag(struct i2c_client *client, char *buf, int bufsize);  //tuwenzan add this sentence at 20150722
...
//tuwenzan add this function at 20150722 begin
...
//tuwenzan add this function at 20150722 end
/*----------------------------------------------------------------------------*/
static int LIS3DH_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	int acc[LIS3DH_AXES_NUM];
	int res = 0;
//tuwenzan remove this at 20150729 begin
//liuying add this at 20150819 begin
        int i = 1;
        int sumx = 0;
        int sumy = 0;
        int sumz = 0;
//liuying add this at 20150819 end
//tuwenzan remove this at 20150729 end
	memset(databuf, 0, sizeof(u8)*10);
  ...
  /*             for(i=1;i<=50;i++)
                ...
                obj->data[LIS3DH_AXIS_X] = sumx/50;
                obj->data[LIS3DH_AXIS_Y] = sumy/50;
                obj->data[LIS3DH_AXIS_Z] = sumz/50;
                if(((obj->data[LIS3DH_AXIS_X]/100)>3)&&((obj->data[LIS3DH_AXIS_X]/100)<8))
                    obj->data[LIS3DH_AXIS_X] = 3*100;
                if(((obj->data[LIS3DH_AXIS_Y]/100)>3)&&((obj->data[LIS3DH_AXIS_Y]/100)<8))
                    obj->data[LIS3DH_AXIS_Y] = 3*100;
                if(((obj->data[LIS3DH_AXIS_Z]/100)>3)&&((obj->data[LIS3DH_AXIS_Z]/100)<8))
                    obj->data[LIS3DH_AXIS_Z] = 3*100;         */
    /*dixiaobing 2015.0508*/
	//wangkangmin add for fix 84592 begin
	//liuying add this at 20150819 begin
	/*
	for(i=1;i<=30;i++)
                {
                ...
                obj->data[LIS3DH_AXIS_X] = sumx/30;
                obj->data[LIS3DH_AXIS_Y] = sumy/30;
                obj->data[LIS3DH_AXIS_Z] = sumz/30;
	*/
	//wangkangmin add for fix 84592 begin
    //liuying add this at 20150819 end
	//tuwenzan remove this at 20150729 end
...
static ssize_t store_status_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if(1 == sscanf(buf, "%d", &direction))
	{
		...
		if(hwmsen_get_convert(obj->hw->direction, &obj->cvt))
		{
...

{
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_READ_SENSORDATA:
			...
			//tuwenzan call LIS3DH_ReadSensorData_mag function at 20150722 begin
			LIS3DH_SetPowerMode(client,true);
			//LIS3DH_ReadSensorData(client, strbuf, LIS3DH_BUFSIZE);
			LIS3DH_ReadSensorData_mag(client, strbuf, LIS3DH_BUFSIZE);
			//tuwenzan call LIS3DH_ReadSensorData_mag function at 20150722 end
			if(copy_to_user(data, strbuf, strlen(strbuf)+1))
			{
				err = -EFAULT;
				break;	  
