...
#define FLASH_GPIO_ENF GPIO_TORCH_EN
#define FLASH_GPIO_ENT GPIO_FLASH_LED_EN
...
static struct i2c_board_info __initdata i2c_LM3642={ I2C_BOARD_INFO(LM3642_NAME, 0x37), \
...
int FL_Enable(void)
{
	  char buf[2];
	  u16 brightness=0x00;
	  //is skyworks ic sky81294
		PK_DBG(" FL_Enable is sky ic = true \n");
		//STROBE_DEVICE_ID = 0x6e;
		if(g_duty==0){//liuying 20140606
		//torch mode
		PK_DBG("%s,%d,torch",__func__,__LINE__);
		buf[0]=0x02;	//movie mode current register
		buf[1]=0x04;	//130 mA   //liuying 20150910
		//flashlight_iWriteRegI2C(buf , 2, STROBE_DEVICE_ID);
		...
		switch(g_duty){
			...
			case 8:
				...
				break;
			case 9:
				brightness = 0x0f;//1000 mA
				break;
			case 10:
				brightness = 0x10;//1110 mA
				break;
			case 11:
				brightness = 0x11; //1215 mA
				break;
			case 12:
				brightness = 0x12; //1315 mA
				break;
			case 13:
				brightness = 0x13; //1415 mA
				break;
			default:
				brightness = 0x0f;
				break;
...
{
		char buf[2],bufR[2];
		//disable sky torch & flash
		buf[0]=0x03;
		//flashlight_iReadRegI2C(buf, 1, bufR, 1, STROBE_DEVICE_ID);
		LM3642_write_reg(LM3642_i2c_client,buf[0],buf[1]);
		PK_DBG(" FL_Enable reg=0x03 value=%x\n",(int)bufR[0]);
	
		buf[0]=0x03;
		buf[1]=bufR[0] & 0xFC;
		PK_DBG(" FL_Enable reg=0x03 buf[1]value=%x\n",buf[1]);
		//flashlight_iWriteRegI2C(buf , 2, STROBE_DEVICE_ID);
		LM3642_write_reg(LM3642_i2c_client,buf[0],buf[1]);
...
static int constant_flashlight_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	PK_DBG("sky81294 constant_flashlight_ioctl() line=%d ior_shift=%d, iow_shift=%d iowr_shift=%d arg=%d\n",__LINE__, ior_shift, iow_shift, iowr_shift,(int)arg);
    switch(cmd)
    {
