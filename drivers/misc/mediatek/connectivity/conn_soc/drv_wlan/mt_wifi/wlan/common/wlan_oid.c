    ...
    )
{
    ...
    *pu4QueryInfoLen = u4QueryBufferLen;
    ...
    if (u4QueryBufferLen != sizeof(PARAM_MTK_WIFI_TEST_STRUC_T)) 
        DBGLOG(REQ, WARN, ("Invalid data. QueryBufferLen: %u.\n",
                    u4QueryBufferLen));
    ...
    )
{
    ...
    *pu4SetInfoLen = sizeof(PARAM_MTK_WIFI_TEST_STRUC_T);
    ...
    if (u4SetBufferLen != sizeof(PARAM_MTK_WIFI_TEST_STRUC_T))
        DBGLOG(REQ, WARN, ("Invalid data. SetBufferLen: %u.\n",
                    u4SetBufferLen));
    ...
    )
{
    ...
    if(u4FuncIndex == RF_AT_FUNCID_FW_INFO) {
        ...
        if (u4QueryBufferLen > 8) {
            ...
            DBGLOG(INIT, INFO,("<wifi> version: 0x%x ,extended : 0x%x\n"
                , prTestStatus->rATInfo.u4FuncData
                , prTestStatus->rATInfo.u4FuncData2));
        } else
            DBGLOG(INIT, INFO, ("<wifi> version: 0x%x\n"
                , prTestStatus->rATInfo.u4FuncData));
    ...
    )
{
    ...
    // increase command sequence number
    ucCmdSeqNum = nicIncreaseCmdSeqNum(prAdapter);
    DBGLOG(REQ, TRACE, ("ucCmdSeqNum =%d, ucCID =0x%x\n", ucCmdSeqNum, ucCID));
