...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
    ...
    epl_sensor.als.cycle = EPL_CYCLE_32;
    ...
    epl_sensor.ps.integration_time = EPL_PS_INTT_520; //liqiang
    epl_sensor.ps.gain = EPL_GAIN_LOW;
#if PS_DYN_K
    dynk_max_ir_data = 50000; // ps max ch0
// hebiao 20151027 begin
    dynk_low_offset = 500;
...
void epl_sensor_reset_dynk_thd(u8 last_status, u8 now_status)
{
    if(last_status==0 && now_status==1)
    {
	    ...
	    APS_LOG("[%s]:restart dynk thre_l = %ld, thre_h = %ld\n", __func__, (long int) dynk_thd_low, (long int) dynk_thd_high);
#endif
        eint_flag = false;
...
void epl_sensor_dynk_thd_polling_work(struct work_struct *work)
{
    ...
    if(enable_ps == true)
    {
...
#endif
        {
            ...
            mutex_unlock(&sensor_mutex);
            eint_flag = true;
		    APS_LOG("[%s]:dyn k thre_l = %ld, thre_h = %ld\n", __func__,(long int ) dynk_thd_low, (long int )dynk_thd_high);//wangxiqiang
        }
        else if(dynk_change_flag==true && (epl_sensor.ps.data.data>dynk_change_thd_max) && ( (epl_sensor.ps.compare_low >> 3)==0 ))
        {
            ...
            mutex_unlock(&sensor_mutex);
            eint_flag = true;
            APS_LOG("[%s]: epl_sensor.ps.data.data=%d, L/H=%ld/%ld \r\n", __func__, epl_sensor.ps.data.data, (long int )dynk_thd_low, (long int )dynk_thd_high);
        }
#if PS_DYN_K_STR
        if(polling_flag == true && eint_flag == true)
        {
...
{
    sscanf(buf, "%d",(int *)&epl_sensor.als.factory.lux_per_count);//wangxiqiang
    return count;
}
