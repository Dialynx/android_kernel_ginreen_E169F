...
char *alsps_name  = NULL;   //add by lichengmin
...
static int alsps_real_driver_init(void) 
{
	...
	for(i = 0; i < MAX_CHOOSE_ALSPS_NUM; i++)
	{
	  ...
	  {
	    printk(" [darren-als] alsps try to init driver %s,%s\n", alsps_init_list[i]->name,__func__);
	    err = alsps_init_list[i]->init();

		printk(" [darren-als] alsps try to init driver err = %d,%s\n", err,__func__);
		
		if(0 == err)
		{
		   printk("  [darren-als] alsps real driver alsps_name = %s probe ok\n", alsps_init_list[i]->name);

		   alsps_name =  alsps_init_list[i]->name;    //add by lichengmin
