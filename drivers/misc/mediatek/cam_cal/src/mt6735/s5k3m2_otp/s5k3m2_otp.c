...
*
********************************************************************************/
//static struct i2c_board_info __initdata kd_cam_cal_dev={ I2C_BOARD_INFO(CAM_CAL_DRVNAME, S5K3M2_OTP_DEVICE_ID>>1)};
...
#define MAX_LSC_SIZE 2048//1024
#define MAX_OTP_SIZE 2048//1100
//static int s5ks5k3m2_otp_read = 0;
...
u8 s5k3m2_eeprom_lsc_data[2048]={0};//liuying 20150310 add
//extern u8 s5k3m2_eeprom_lsc_data[];
...
int read_s5k3m2_otp(u8 page, u16 offset, u8* data)
{
...
int read_s5k3m2_otp_size(u8 page, u16 offset, u8* data,int size)
{
	int i = 0;
	for(i = 0; i < size; i++){
		if(!read_s5k3m2_otp(page, offset+i, data+i))
			return -1;
	}
    return 0;
}
...
#if 0
    u8 data[9];
	read_s5k3m2_otp_size(0x01,0x3B04,&data[0],9);
#endif

	//read calibration version 0xff000b01
	read_s5k3m2_otp_size(0x01,0x3B0D,&s5ks5k3m2_otp_data.Data[0x01],4);

	//read serial number
	read_s5k3m2_otp_size(0x01,0x3B11,&s5ks5k3m2_otp_data.Data[0x05],2);

	//read AF config
	read_s5k3m2_otp_size(0x01,0x3B13,&s5ks5k3m2_otp_data.Data[0x07],2);

	//read AWB
	read_s5k3m2_otp_size(0x01,0x3B15,&s5ks5k3m2_otp_data.Data[0x09],8);

	//read AF calibration
	read_s5k3m2_otp_size(0x01,0x3B1D,&s5ks5k3m2_otp_data.Data[0x011],4);

    //read LSC size
	read_s5k3m2_otp_size(0x01,0x3B21,&s5ks5k3m2_otp_data.Data[0x015],2);

    //for lsc data
    offset = 0x17;
    for(i = 2; i<0x10; i++ ){
		read_s5k3m2_otp_size(i,0x3B04, &s5ks5k3m2_otp_data.Data[offset],64);
		offset += 64;
    }
	read_s5k3m2_otp_size(i,0x3B04,&s5ks5k3m2_otp_data.Data[offset],36);
	return 0;
}
#endif

#if 1 //liuying 20150311 begin
...
static bool read_3m2_eeprom_byte(kal_uint16 addr, BYTE* data, kal_uint32 size ){
	kal_uint32 i = 0;
	kal_uint16 offset = addr;
	for(i = 0; i < size; i++) {
		if(!read_eeprom_byte(offset, &data[i])){
			return false;
		}
		CAM_CALDB("read_eeprom_byte 0x%0x %d\n",offset, data[i]);
		offset++;
	}
	//get_done = true;
	//last_size = size;
...
int  read_3m2_eeprom_for_mtk_new(void ){
	kal_uint32 i;
	kal_uint16 addr = 0x0000;
	kal_uint32 size =  1894;//5;//liuying 20150310 total 1894 byte
	
	CAM_CALDB("check s5k3m2 OTP readed =%d \n",s5k3m2_otp_read_lsc);

	if(1 == s5k3m2_otp_read_lsc ) {
		CAM_CALDB("liuying OTP have readed ! skip\n");
		return 0;
	}
	spin_lock(&g_CAM_CALLock);
	...
	read_3m2_otp_size(0x001A,&s5k3m2_otp_data.Data[0x015],1868);
      #else
	//for(i = 0; i < size; i++){
	//	if(!read_3m2_eeprom_byte(addr, s5k3m2_eeprom_lsc_data, size))
	//		return -1;
	//}
	for(i = 0; i < size; i++){
		s5k3m2_eeprom_lsc_data[i]=read_cmos_sensor_8(addr);
		addr++;
		CAM_CALDB("check s5k3m2 OTP readed data[%d]=0x%x \n",i,s5k3m2_eeprom_lsc_data[i]);
	}
      #endif
...
static long s5ks5k3m2_otp_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    long ret;
    COMPAT_stCAM_CAL_INFO_STRUCT __user *data32;
    stCAM_CAL_INFO_STRUCT __user *data;
...
static const struct file_operations g_stCAM_CAL_fops =
{
    ...
    //.ioctl = CAM_CAL_Ioctl
#ifdef CONFIG_COMPAT
    .compat_ioctl = s5ks5k3m2_otp_Ioctl_Compat,
#endif
