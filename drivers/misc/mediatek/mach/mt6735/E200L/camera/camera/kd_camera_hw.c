...
//zhenglihong on 20150707 for poweron add begin
	else if ((pinSetIdx == 0 ) && currSensorName && (0 == strcmp(currSensorName, SENSOR_DRVNAME_S5K3M2_MIPI_RAW)))
        {
...
//liqiang 20150717 end
		...
		PK_DBG("kdCISModulePowerOn get in---  SENSOR_DRVNAME_OV5670_MIPI_RAW \n");
            ...
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1200,mode_name))//changed by zhenglihong@20150727
            {
                 PK_DBG("[CAMERA SENSOR] Fail to enable digital power (VCAM_D), power id = %d \n", CAMERA_POWER_VCAM_D);
                 goto _kdCISModulePowerOn_exit_;
            }
			...
			else if ((pinSetIdx == 1 ) && currSensorName && (0 == strcmp(SENSOR_DRVNAME_HI551_MIPI_RAW, currSensorName)))
        {
            //First Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
...
//liqiang 20150717 end
        ...
        {
            PK_DBG("[PowerOFF] get in---  SENSOR_DRVNAME_OV5670_MIPI_RAW \n");
            //Set Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
                if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! (CMPDN)\n");}
			...
			//zhenglihong on 20150707 for poweroff add begin
			else  if ((pinSetIdx == 0 ) && currSensorName && (0 == strcmp(currSensorName,SENSOR_DRVNAME_S5K3M2_MIPI_RAW)))
        {
...
//zhenglihong 20150819 begin
	...
	#endif
//zhenglihong 20150819 end

        }
		//zhenglihong on 20150707 for poweroff add end
        else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC2355_MIPI_RAW, currSensorName)))
        {
		//liqiang 20150717 begin
           // mt_set_gpio_out(GPIO_SPI_MOSI_PIN,GPIO_OUT_ZERO);
