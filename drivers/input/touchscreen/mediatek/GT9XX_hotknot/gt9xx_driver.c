...
#include "../../../../include/mach/battery_common.h"     //tuwenzan add at 20150930
...
static u32 gesture_control_value[4] = {0x0,0x0,0x0,0x0};
...
#endif
//huangzhaosong add device info at 20150911 begin 
   ...
   #endif  
//huangzhaosong add device info at 20150911 end 
    return 0;
}
...
#endif
*/
		chr_status = bat_is_charger_exist();
    printk("tuwenzan chr_status = %d\n",chr_status);
    if (chr_status)     // charger plugged in
    {
    	printk("tuwenzan charger plugged in\n");
    ...
    {
    	printk("tuwenzan charger plugged out\n");
        if (chr_pluggedin || dir_update)
        {
            chr_cmd[2] = 7;
            ret = gtp_i2c_write(i2c_client_point, chr_cmd, 3);
...
static int touch_event_handler(void *unused)
{
    ...
    do
    {
		...
		GTP_DEBUG("[darren-gtp] gesture_value = 0x%x\n",gesture_value);
		GTP_DEBUG("[darren-gtp] gesture_control_value[0] = 0x%x\n",gesture_control_value[0]);
		GTP_DEBUG("[darren-gtp] gesture_control_value[1] = 0x%x\n",gesture_control_value[1]);
		GTP_DEBUG("[darren-gtp] gesture_control_value[2] = 0x%x\n",gesture_control_value[2]);
		GTP_DEBUG("[darren-gtp] gesture_control_value[3] = 0x%x\n",gesture_control_value[3]);
	...
	{               
		...
		{
		    case 'c':
			if(gesture_control_value[2] == 0x04 && gesture_control_value[0] == 0x02)
			{
	            		GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_C");
				doze_status = DOZE_WAKEUP;
				input_report_key(tpd->dev, UNICODE_C, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, UNICODE_C, 0);
				input_sync(tpd->dev);
		    ...
		    case 'o':
			if(gesture_control_value[2] == 0x04 && gesture_control_value[0] == 0x80)
			{
	            		GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_O");
				doze_status = DOZE_WAKEUP;
				input_report_key(tpd->dev, UNICODE_O, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, UNICODE_O, 0);
				input_sync(tpd->dev);
		    ...
		    case 'm':
			if(gesture_control_value[2] == 0x04 && gesture_control_value[0] == 0x10)
			{
	            		GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_M");
				doze_status = DOZE_WAKEUP;
				input_report_key(tpd->dev, UNICODE_M, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, UNICODE_M, 0);
				input_sync(tpd->dev);
		    ...
		    case 'e':
			if(gesture_control_value[2] == 0x04 && gesture_control_value[0] == 0x04)
			{
	            		GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_E");
				doze_status = DOZE_WAKEUP;
				input_report_key(tpd->dev, UNICODE_E, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, UNICODE_E, 0);
				input_sync(tpd->dev);
		    ...
		    break;
		    case 'w':
			if(gesture_control_value[2] == 0x04 && gesture_control_value[0] == 0x08)
			{
	            		GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_W");
				doze_status = DOZE_WAKEUP;
				input_report_key(tpd->dev, UNICODE_W, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, UNICODE_W, 0);
				input_sync(tpd->dev);
		    ...
		    break;
		    case 's':
			if(gesture_control_value[2] == 0x04 && gesture_control_value[0] == 0x20)
			{
	            		GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_S");
				doze_status = DOZE_WAKEUP;
				input_report_key(tpd->dev, UNICODE_S, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, UNICODE_S, 0);
				input_sync(tpd->dev);
		    ...
		    break;
		    case 'z':
			if(gesture_control_value[2] == 0x04 && gesture_control_value[0] == 0x40)
			{
	            		GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_Z");
				doze_status = DOZE_WAKEUP;
				input_report_key(tpd->dev, UNICODE_Z, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, UNICODE_Z, 0);
				input_sync(tpd->dev);
		    ...
		    break;
		    case 'v':
			if(gesture_control_value[2] == 0x04 && gesture_control_value[0] == 0x01)
			{
	            		GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_V");
				doze_status = DOZE_WAKEUP;
				input_report_key(tpd->dev, UNICODE_V_UP, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, UNICODE_V_UP, 0);
				input_sync(tpd->dev);
			...
			if(gesture_control_value[2] == 0x04 && gesture_control_value[0] == 0x01)
			{
				...
				doze_status = DOZE_WAKEUP;
				input_report_key(tpd->dev, UNICODE_V_DOWN, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, UNICODE_V_DOWN, 0);
				input_sync(tpd->dev);
				// clear 0x814B
				doze_buf[2] = 0x00;
		...
		case '>':
			if(gesture_control_value[2] == 0x04 && gesture_control_value[0] == 0x01)
			{
	            		GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_>>>>");
				doze_status = DOZE_WAKEUP;
				input_report_key(tpd->dev, UNICODE_V_L, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, UNICODE_V_L, 0);
				input_sync(tpd->dev);
		    ...
		    case 0xAA:
			if(gesture_control_value[2] == 0x03 && gesture_control_value[0] == 0x01)
			{
	            		GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_RIGHT");
				doze_status = DOZE_WAKEUP;
				input_report_key(tpd->dev, SWIPE_X_RIGHT, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, SWIPE_X_RIGHT, 0);
				input_sync(tpd->dev);
		    ...
		    case 0xBA:
			if(gesture_control_value[2] == 0x03 && gesture_control_value[0] == 0x08)
			{
	            		GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_UP");
				doze_status = DOZE_WAKEUP;
				input_report_key(tpd->dev, SWIPE_Y_UP, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, SWIPE_Y_UP, 0);
				input_sync(tpd->dev);
		    ...
		    case 0xBB:
			if(gesture_control_value[2] == 0x03 && gesture_control_value[0] == 0x02)
			{
	            		GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_LEFT");
				doze_status = DOZE_WAKEUP;
				input_report_key(tpd->dev, SWIPE_X_LEFT, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, SWIPE_X_LEFT, 0);
				input_sync(tpd->dev);
		 ...
		 case 0xAB:
		 	if(gesture_control_value[2] == 0x03 && gesture_control_value[0] == 0x04)
			{
	            		GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_DOWN");
				doze_status = DOZE_WAKEUP;
				input_report_key(tpd->dev, SWIPE_Y_DOWN, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, SWIPE_Y_DOWN, 0);
				input_sync(tpd->dev);
		    ...
		    case 0xCC:
			if(gesture_control_value[2] == 0x02 && gesture_control_value[0] == 0x01)
			{
	            		GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_DOUBLE_TAP");
				doze_status = DOZE_WAKEUP;
				input_report_key(tpd->dev, DOUBLE_TAP, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, DOUBLE_TAP, 0);
				input_sync(tpd->dev);
...
/**modify by yudengwu 2015-01.7**/
    ...
    if (gesture_control_value[1] == 0x01 && gesture_control_value[0] == 0x00)
    {			
		mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
...
//	 sprintf(buf, "node test %d\n",test_value );

//	    if (sscanf(regBuf, "%x %x",  &sensorReg.RegAddr, &sensorReg.RegData) == 2) {

		return sprintf(buf, "node test 0x%x,0x%x,0x%x,0x%x\n", gesture_control_value[0], gesture_control_value[1] ,gesture_control_value[2] ,gesture_control_value[3]);
...
//	sscanf(buf, "%d\n",&test_value);
	...
	sscanf(buf, "0x%x,0x%x,0x%x,0x%x\n",&gesture_control_value[0],&gesture_control_value[1],&gesture_control_value[2],&gesture_control_value[3]);
	return size;
}
