...
static  void tpd_down(int x, int y,int press,int area, int p) {
	...
	if ((!press) && (!p))
       {
	    ...
	    printk("tpd:D[100,100]\n ");
       }
       else
       {
           input_report_abs(tpd->dev, ABS_MT_PRESSURE, press);//add by sven
           input_report_abs(tpd->dev, ABS_MT_TOUCH_MAJOR, area);//add by sven
           /* track id Start 0 */
	...
	printk("tpd:D[%4d %4d %4d %4d %4d]\n ", x, y, p,press,area);//add by sven
	 /* track id Start 0 */
     //input_report_abs(tpd->dev, ABS_MT_TRACKING_ID, p); 
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
	...
	for(i = 0; i < point_num; i++)  
	{
		...
		cinfo->pressure[i] =(data[7 + 6 * i]);//add by sven
		
 		cinfo->area[i] = (data[8 + 6 *1]);//add by sven
	}

	printk(" MIKE: tpd cinfo->x[0] = %d, cinfo->y[0] = %d, cinfo->p[0] = %d, cinfo->pressure[0] = %d, cinfo->area[0] = %d, \n", 
                  cinfo->x[0], cinfo->y[0], cinfo->p[0], cinfo->pressure[0], cinfo->area[0]);//add by sven
	return true;
...
static int fts_read_Touchdata(struct ts_event *pinfo)
{
	...
	pinfo->touch_point = 0;
	pinfo->touch_point_num=buf[2] & 0x0F;
	...
	for (i = 0; i < fts_updateinfo_curr.TPD_MAX_POINTS; i++)
	{
		...
		pinfo->pressure[i] = (buf[7 + 6 * i]);//cannot constant value
		pinfo->area[i] = (buf[8 + 6 * i]) >> 4;
		 ...
		 if (event->au8_touch_event[i]== 0 || event->au8_touch_event[i] == 2)
			 {
				 ...
				 input_report_abs(tpd->dev, ABS_MT_PRESSURE,event->pressure[i]);   //0x3f
				 input_report_abs(tpd->dev, ABS_MT_TOUCH_MAJOR,event->area[i]);    //0x05
				 ...
				 event->touchs |= BIT(event->au8_finger_id[i]);
              			//printk("tpd D x[%d] =%d,y[%d]= %d,pressure[%d]= %d,area[%d]= %d",i,event->au16_x[i],i,event->au16_y[i],event->pressure[i],event->area[i]);
			 }
			 else
			 {
				 up_point++;
...

	{
		...
		if (tpd_touchinfo(&cinfo, &pinfo)) 
		{
			...
			if(point_num >0) 
			{
			    ...
			    {
				 tpd_down(cinfo.x[i], cinfo.y[i],cinfo.pressure[i],cinfo.area[i], cinfo.id[i]);
			    }
			    input_sync(tpd->dev);
			} else {
 ...
 ***********************************************************************/
 static void gtp_esd_check_func(struct work_struct *work)
 {
	 ...
	 if (tpd_halt ) 
	 {
		printk("[FTS]tpd_halt=1, tpd_halt return/n");
		return;
	 }
	 if(apk_debug_flag) 
	 {
		 //queue_delayed_work(gtp_esd_check_workqueue, &gtp_esd_check_work, esd_check_circle);
		printk("[FTS]apk_debug_flag =1,tpd_halt return/n");
		return;
	 }
	 ...
	 // esd check for count
	 ret = i2c_smbus_read_i2c_block_data(i2c_client, 0x8F, 1, &data);
	 ...
	 ret = i2c_smbus_read_i2c_block_data(i2c_client, 0x91, 1, &data);
...
/*release all touch points*/
 ...
 static void tpd_suspend( struct early_suspend *h )
 {
	...
	if (tpd_proximity_flag == 1)
	{
	 ...
	 mutex_lock(&i2c_access);
	 TPD_DMESG("TPD LIUPENG done\n");
#ifdef TPD_CLOSE_POWER_IN_SLEEP	
	hwPowerDown(TPD_POWER_SOURCE,"TP");
#else

	//i2c_smbus_write_i2c_block_data(i2c_client, 0xA5, 1, &data);  //TP enter sleep mode
	ret = fts_write_reg(i2c_client, 0xA5,data);//TP enter sleep mode
	if (ret<0) 
	{
		printk("[Focal][Touch] write value fail");
	}
