...
//huangzhaosong 2015116 s----
#ifndef TP_VENDOR_ID_CUST
#define TP_VENDOR_ID_CUST 0x79
#endif

#ifdef TP_UPDATE_BIN_CUST
static unsigned char CTPM_FW[] = {
	#include TP_UPDATE_BIN_CUST
};
#else
static unsigned char CTPM_FW[] = {
	#include "E168_YUDE_5x46_V0b_D01_20150805_app.i" ////tuwenzan modify at 20151209
};
#endif
//huangzhaosong 2015116 e----
...
//tuwenzan modify at 20151209 begin
static unsigned char ft5x36_ctpm_VidFWid_get_from_boot(  struct i2c_client *client )
{
	unsigned char auc_i2c_write_buf[10];
	unsigned char reg_val[4] = {0};
	unsigned char i = 0;
	...
	for(i = 0;i < FTS_UPGRADE_LOOP; i++) 
	{
		auc_i2c_write_buf[2] = 0x07;//0x07
		auc_i2c_write_buf[3] = 0xb3;//b4
		i_ret = fts_i2c_Write(client, auc_i2c_write_buf, 4);
		if (i_ret < 0)
		{
			FTS_DBG( "[FTS] Step 4: read vendor id from flash error when i2c write, i_ret = %d\n", i_ret);
...
//tuwenzan modify at 20151209 end
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	if(uc_tp_vendor_id != TP_VENDOR_ID_CUST)// default , TP Vendor id
	{
		uc_boot_vendor_id = ft5x36_ctpm_VidFWid_get_from_boot(client); //tuwenzan modify at 20151209
		DBG("[FTS] uc_boot_vendor_id= 0x%x!\n", uc_boot_vendor_id);
		if(uc_boot_vendor_id == TP_VENDOR_ID_CUST)
		{
