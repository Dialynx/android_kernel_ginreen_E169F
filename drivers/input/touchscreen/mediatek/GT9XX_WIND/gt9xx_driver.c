...
static ssize_t gesture_read(struct device* cd,struct device_attribute *attr, char* buf);
static ssize_t gesture_write(struct device* cd, struct device_attribute *attr,const char* buf, size_t len);
static DEVICE_ATTR(gesture, S_IWUGO | S_IRUGO , gesture_read, gesture_write);
#endif
...
#if GTP_GESTURE_WAKEUP
const u16 gesture_key_array[] = {KEY_F13,KEY_F14,KEY_F15,KEY_F16,KEY_F17,KEY_F18,KEY_F19,KEY_F20,KEY_F21};
...
static const struct i2c_device_id tpd_i2c_id[] = {{"gt9xx-ts", 0}, {}};
static unsigned short force[] = {0, 0xBA, I2C_CLIENT_END, I2C_CLIENT_END};
static const unsigned short *const forces[] = { force, NULL };
//static struct i2c_client_address_data addr_data = { .forces = forces,};
static struct i2c_board_info __initdata i2c_tpd = { I2C_BOARD_INFO("gt9xx-ts", (0xBA >> 1))};
static struct i2c_driver tpd_i2c_driver =
{
    .probe = tpd_i2c_probe,
    .remove = tpd_i2c_remove,
    .detect = tpd_i2c_detect,
    .driver.name = "gt9xx-ts",
    .id_table = tpd_i2c_id,
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
    ...
    while (retry++ < 1) //liqiang 20150527 modified
    {
        ...
        //msleep(10); //liqiang 20150527 modified
    }
...
static ssize_t gesture_write(struct device* cd, struct device_attribute *attr,
...
static int touch_event_handler(void *unused)
{
    ...
    do
    {
        ...
        if (DOZE_ENABLED == doze_status && gesture_flag == 1)
        {
           ...
           // GTP_INFO2("0x814B = 0x%02X", doze_buf[2]);
            GTP_DEBUG("0x814B = 0x%02X", doze_buf[2]);
            if (ret > 0)
            {     
                #if 0
                if ((doze_buf[2] == 'a') || (doze_buf[2] == 'b')|| (doze_buf[2] == 'd') || (doze_buf[2] == 'g') || 
                    (doze_buf[2] == 'h') ||(doze_buf[2] == 'q') || (doze_buf[2] == 'y') || (doze_buf[2] == 0x5E) /* ^ */
                    )
                {
                    if (doze_buf[2] != 0x5E)
                    {
				...
				{
                    GTP_INFO("liukun gesture s!");
                    //doze_status = DOZE_WAKEUP;
                    input_report_key(tpd->dev, KEY_F18, 1);
				...
				{
                    GTP_INFO("liukun gesture z!");
                    //doze_status = DOZE_WAKEUP;
                    input_report_key(tpd->dev, KEY_F19, 1);
				...
				{
                    GTP_INFO("liukun gesture v!");
                    //doze_status = DOZE_WAKEUP;
                    input_report_key(tpd->dev, KEY_F20, 1);
                ...
                else if (0xCC == doze_buf[2])
                {
                    ...
                    //doze_status = DOZE_WAKEUP;
                    input_report_key(tpd->dev, KEY_F21, 1);
...
//liqiang  20150304 begin 
    #ifdef TPD_HAVE_BUTTON
        key_value = point_data[3 + 8 * touch_num];
//	GTP_INFO2("%s,%d,key_value=%x\n",__func__,__LINE__,key_value);	
        ...
        if (key_value || pre_key)
        {
			...
			//		GTP_INFO2("button =%d %d",input_x,input_y);
					GTP_DEBUG("button =%d %d",input_x,input_y);
				   
					tpd_down( input_x, input_y, 40, 0);
				}
